...
import { Droplets, Leaf, Wind, Flame, Sun } from 'lucide-react'

const EnhancedMap = dynamic(() => import('./enhanced-map'), { ssr: false })
  ...
  useEffect(() => {
    ...
    simulationRef.current = forceSimulation(initializedAppliances)
      ...
      .force('center', forceCenter(width / 2, height / 2)
        .strength(0.05) // Reduced center pull for more drift
      )
      .force('x', forceX(width / 2).strength(0.01)) // Very weak X pull
      .force('y', forceY(height / 2).strength(0.01)) // Very weak Y pull
      .velocityDecay(0.02) // Very low friction for "space-like" drift
      .alpha(1)
      .alphaDecay(0)
    ...
    const updateNodesPosition = () => {
      if (simulationRef.current) {
        ...
        simulationRef.current.nodes().forEach(node => {
          ...
          // Soft boundaries - push back if getting too close to edge
          const radius = calculateBubbleSize(node.energyConsumption) / 2;
          const margin = 50;

          if (node.x < radius + margin) node.vx += 0.5;
          if (node.x > width - radius - margin) node.vx -= 0.5;
          if (node.y < radius + margin) node.vy += 0.5;
          if (node.y > height - radius - margin) node.vy -= 0.5;
        });
